...
- **AI 增强**：由 AI 生成通知的标题前缀/tag，以及回合通知附带的 Markdown 总结正文
...
```bash
dsh plugin --profile desktop add dsh-plugin-ntfy   # 或本地路径
```

或手动：在 `~/.dsh/profiles/<profile>/package.json` 的 `dependencies` 里加
`"dsh-plugin-ntfy": "file:../.."`（或版本号），把 `"dsh-plugin-ntfy"` 加入
...
- 认证（自建实例启用了 access control 时填）：
  - `username` + `passwordEnv`：HTTP Basic（`Authorization: Basic base64(user:pass)`）
  - `accessTokenEnv`：Bearer（`Authorization: Bearer tk_...`），优先于用户名密码
  - 密码/token 字段可直接填字面量；填**全大写环境变量名**（如 `NTFY_PASSWORD`）则从 DSH credentials / 进程环境读取
- `timeoutMs`：请求超时，默认 10000
- `includeTurnSummary`：回合通知是否附带 AI 生成的 Markdown 总结正文，默认开
- 四个事件开关：`notifyOnTurnCompletion` / `notifyOnTurnFailure` / `notifyOnJobCompletion` / `notifyOnJobFailure`，默认全开
...
agent 可用工具：`ntfy_notify`（主动发一条通知，可带 message/title/priority/tags/topic 参数）。
...
- `src/analyze.ts` — AI 分析：生成通知标题前缀/tag，以及回合的 Markdown 总结正文
